...
void cpu_physical_memory_rw(target_phys_addr_t addr, uint8_t *buf,
                            int len, int is_write, char* devname) {
    // GA FAROS
    if (devname && !strncmp("e1000", devname, 5)) {
        /*fprintf(stderr, "CPU_PHYS_MEM_RW: dev: %s\taddr: 0x%016x\tlen: %d\t\tis_write: %d\n",*/
                /*devname, addr, len, is_write);*/
        panda_cb_list *plist;
        if (is_write) {
            for (plist = panda_cbs[PANDA_CB_FAROS_NET_RECV];
                    plist != NULL;
                    plist = panda_cb_list_next(plist)) {
                plist->entry.faros_net_recv(cpu_single_env, addr, len);
            }
        } else {
            for (plist = panda_cbs[PANDA_CB_FAROS_NET_SEND];
                    plist != NULL;
                    plist = panda_cb_list_next(plist)) {
                plist->entry.faros_net_recv(cpu_single_env, addr, len);
            }
                        ...
                        uint8_t *buf, int len, int is_write)
{
    ...
    while (len > 0) {
        ...
        if (is_write)
            ...
            cpu_physical_memory_rw(phys_addr, buf, l, is_write, /* GA FAROS */ NULL);
        len -= l;
        buf += l;
        addr += l;
    }
