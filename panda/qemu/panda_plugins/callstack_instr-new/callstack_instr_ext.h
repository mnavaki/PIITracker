...
typedef int(*get_callers_t)(target_ulong *callers, int n, CPUState *cpu);
static get_callers_t __get_callers = NULL;
static inline int get_callers(target_ulong *callers, int n, CPUState *cpu);
static inline int get_callers(target_ulong *callers, int n, CPUState *cpu){
    return __get_callers(callers,n,cpu);
}
typedef int(*get_functions_t)(target_ulong *functions, int n, CPUState *cpu);
static get_functions_t __get_functions = NULL;
static inline int get_functions(target_ulong *functions, int n, CPUState *cpu);
static inline int get_functions(target_ulong *functions, int n, CPUState *cpu){
