...
typedef struct taintLocation{
    uint32_t addr;
    uint32_t size;
    uint32_t type;
    struct taintLocation *next;
}taintLocation;

enum packetType {TCP=1, UDP};

typedef struct packetInfo{
    char         *destIP;
    uint32_t     destPort;
    target_ulong buffAddr;
    uint32_t     buffSize;
    packetType   pt;
}packetInfo;


enum types {MAC_ADDR_LABEL=1, COMPUTER_NAME_LABEL, HOST_NAME_LABEL, 
