...
txdesc_writeback(E1000State *s, dma_addr_t base, struct e1000_tx_desc *dp)
{
    ...
    /*fprintf(stderr, "Got network upper DMA @ 0x%08x [0x%x]\n", */
            /*(uint64_t)(&dp->upper), sizeof(dp->upper));*/
    if (rr_in_record()){
...
e1000_receive(VLANClientState *nc, const uint8_t *buf, size_t size)
{
    ...
    do {
        ...
        if (desc.buffer_addr) {
            if (desc_offset < size) {
                ...
                pci_dma_write(&s->dev, le64_to_cpu(desc.buffer_addr),
                                 ...
                                 copy_size);
                /*fprintf(stderr, "Got network buf DMA @ 0x%08x [0x%x]\n", */
        ...
        /*fprintf(stderr, "Got network desc DMA @ 0x%08x [0x%x]\n", */
                /*base, sizeof(desc));*/
